...
def basic_queries(request):
    try:
        ...
        exclude_books = Book.objects.exclude(price__lt=600)
...
def raw_sql_demo(request):
    books = Book.objects.raw('SELECT id, title, price FROM my_newApp_book WHERE price > %s', [300])
...
@login_required
@permission_required('my_newApp.add_blogpermissionpost',raise_exception=True)# when we make migrations then permissions will be created by using this line of code
...
@permission_required('my_newApp.change_blogpermissionpost',raise_exception=True)
...
@login_required
@permission_required('my_newApp.delete_blogpermissionpost',raise_exception=True)
...
def register_auth(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect("login")# after creating user it redirects to login form
    else:
...
@login_required #It is a decorator used to the code only if required of using code
...
def register(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            #return render(request,"success.html",{"username":form.cleaned_data['username']})
    else:
...
#def home(request):
    #product_name= "Ed Silk Sarees"
    # return render(request,"home.html", {'product_name':product_name})

    # price=100
    # tax=80
    # return render(request,"home.html", {'price':price, 'tax':tax})


#def home(request):
    # context={
    #     'user_name':'Elavarasan',
    #     'items':['silk sarees','silk shirts','silk lungies'],
    # }
...
def post_detail(request):
    post={"title": "My Blog", "created_at":datetime(2026,1,24,10,22)}
...
def welcome(request):
    user = {"is_authenticated": False, "username":"deva"}
